...
def gen_one_item_cand_set(input_fileator):
    ...
    for record in input_fileator:
        ...
        for item in transaction:
            new_set = set()
            new_set.add(item)
            if frozenset(new_set) not in one_cand_set:
                one_cand_set.add(frozenset(new_set))
        #========================#
        #   END YOUR CODE HERE   #
        #========================# 
    ...
    for key, value in islice(freq_map.items(),1,None):
        for item in value:
            ...
            for element in _subsets:
               ...
               # STRART YOUR CODE HERE  #
               #========================#
                    confidence = float(set_count_map[element.union(remain)]) / float(set_count_map[element])
               #========================#
               #   END YOUR CODE HERE   #
               #========================# 
    ...
    for set_one in s:
        for set_two in s:
            if set_one is not set_two:
                joint_set = set_one.union(set_two)
            
                if len(joint_set) == l:
                    join_set.add(joint_set)
    #========================#
    #   END YOUR CODE HERE   #
    #========================# 
    return join_set
